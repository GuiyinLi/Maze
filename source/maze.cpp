 ...
 * @LastEditTime : 2019-12-29 12:39:59
 * @Description: README
 * @FilePath: \Maze\source\maze.c
 */
#include "maze.h"
#include "stack.h"
#include "queue.h"
...
Status MazePath(mazeInfoPtr maze,RouteInfoPtr r)
{
    ...
    do{
        if(Pass(curpos,maze->mazeMap)){  //当前位置可以通过，即是未曾走到过的通道块
...
directionType operator++(directionType &di,int)
{
	switch(di)
	{
		case east:		di = south;break;
		case south:		di = west;break;
		case west:		di = north;break;
		case north:		di = error;break;
	}
	return di;
}
directionType Transfrom(int d)
{
	...
	switch(d)
	{
		case east:		di = east;break;
		case south:		di = south;break;
		case west:		di = west;break;
		case north:		di = north;break;
	}
	return di;
}
